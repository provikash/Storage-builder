...
async def search_files_command(client: Client, message: Message):
    """Handle /search command for searching files"""
    try:
        ...
        try:
            from bot.database.index_db import search_files
            files = await search_files(query, limit=50, clone_id=clone_id)
        except ImportError:
            try:
                from bot.database.mongo_db import search_clone_files
                files = await search_clone_files(clone_id, query, limit=50)
            except:
                await message.reply_text("❌ Search functionality is currently unavailable.")
                return
        ...
        if not files:
            await message.reply_text(
                ...
                f"• Browse random files: /random",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🎲 Try Random Files", callback_data="clone_random_files")],
                    [InlineKeyboardButton("🏠 Home", callback_data="back_to_start")]
                ])
            )
            return
        ...
        buttons = create_file_buttons(files, current_mode="search")
        await message.reply_text(text, reply_markup=buttons)

        logger.info(f"Search query '{query}' from user {message.from_user.id} returned {len(files)} results")
